...
export default function Dropdown({ button, options, right, onClick, className, btnClassName, disabled }: Props) {
  return (
    <Menu as="div" className={clsx(className, "relative inline-block text-left")}>
      <div>
        <Menu.Button
          ...
          onClick={onClick}
          className={clsx(
            btnClassName ?? "inline-flex w-full justify-center rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 shadow-sm",
            disabled
              ? "cursor-not-allowed bg-gray-100"
              : "bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-theme-500 focus:ring-offset-2 focus:ring-offset-gray-100"
          )}
        >
          {button}
          {!btnClassName && (
